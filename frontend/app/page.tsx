...
import { Star, MessageSquare, Plus, PlusCircle, Globe, Award, Sparkles, X, Heart, Milk, Lock, LogIn, UserPlus, Mail } from "lucide-react";
...
export default function Home() {
  ...
  const fetchShops = async () => {
    try {
      ...
      console.error(err);
      setError("Failed to fetch shops. Make sure your FastAPI backend is running!");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddReview = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/shops/${selectedShop.id}/reviews`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          ...(token ? { "Authorization": `Bearer ${token}` } : {})
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-[#eddcd2]/80 bg-[#fdfaf6]/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-[#2d2219] flex items-center gap-1.5">
                BobaRanker <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-[#e6ccb2] text-[#8c6239]">MVP</span>
              </h1>
              <p className="text-xs text-[#7f6a5b] hidden sm:block">The ultimate guide to bubble tea rankings</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {token ? (
              <>
                <span className="text-xs text-[#7f6a5b] font-medium hidden md:inline">Logged in as: <strong className="text-[#2d2219]">{userEmail}</strong></span>
                <button
        ...
        <div className="flex-1">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3">
            <div>
              <h2 className="text-2xl font-bold text-[#2d2219] flex items-center gap-2">
                <Award className="w-6 h-6 text-[#8c6239]" /> Top Bubble Tea Shops
              </h2>
              <p className="text-sm text-[#7f6a5b]">Ranked by customer reviews and ratings</p>
            </div>
          </div>
          ...
          {error && (
            <div className="p-4 bg-amber-50 border border-amber-200 text-[#8c6239] rounded-xl mb-6 text-sm flex flex-col gap-2 shadow-sm">
              ...
              <span>Backend operations are active. View rankings below or log in to submit entries.</span>
            </div>
          )}
          ...
          {loading ? (
            <div className="grid gap-4">
              {[1, 2, 3].map((n) => (
                <div key={n} className="h-28 rounded-2xl bg-white border border-[#eddcd2]/50 animate-pulse" />
              ))}
            </div>
          ) : shops.length === 0 ? (
            <div className="text-center py-16 bg-white border border-[#eddcd2] rounded-2xl shadow-sm">
              ...
              <h3 className="text-lg font-bold text-[#2d2219]">No shops registered yet</h3>
              <p className="text-sm text-[#7f6a5b] mt-1 max-w-md mx-auto px-4">
                Be the first to add a bubble tea shop to BobaRanker!
              </p>
              {token ? (
                <button
                  onClick={() => setIsAddShopOpen(true)}
                ...
                return (
                  <div
                    ...
                    className={`tea-card p-5 rounded-2xl bg-white cursor-pointer relative overflow-hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${
                      isSelected ? "ring-2 ring-[#8c6239] border-transparent" : ""
                    }`}
                  >
                    {/* Rank Badge */}
                    ...
                    <div className="pl-12 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-lg font-bold text-[#2d2219]">{shop.name}</h3>
                      ...
                      {shop.favorite_drink && (
                        <p className="text-sm text-[#7f6a5b] mt-1.5 flex items-center gap-1.5">
                          ...
                          Recommended: <span className="font-medium text-[#2d2219]">{shop.favorite_drink}</span>
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 pl-12 sm:pl-0">
                      <div className="text-right">
                        ...
                        <p className="text-xs text-[#7f6a5b] flex items-center gap-1 mt-0.5 justify-end">
                          <MessageSquare className="w-3.5 h-3.5" /> {shop.review_count} reviews
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {selectedShop ? (
            <div className="bg-white rounded-2xl border border-[#eddcd2] p-6 shadow-sm sticky top-24">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-bold text-[#2d2219]">{selectedShop.name}</h3>
              ...
              <div className="flex items-center gap-2 py-3 border-y border-[#eddcd2]/50 mb-4 justify-between">
                <div>
                  <div className="flex items-center gap-1 text-amber-500">
                    <Star className="w-5 h-5 fill-amber-500" />
                    <span className="font-bold text-lg text-[#2d2219]">
                      {selectedShop.average_rating > 0 ? selectedShop.average_rating.toFixed(1) : "N/A"}
                    </span>
                  </div>
                  <p className="text-xs text-[#7f6a5b] mt-0.5">Average Rating</p>
                </div>
                {token ? (
                  <button
              ...
              <h4 className="text-sm font-bold text-[#2d2219] uppercase tracking-wider mb-3">Reviews</h4>
              ...
              {reviews.length === 0 ? (
                ...
                <div className="grid gap-3 max-h-[350px] overflow-y-auto pr-1">
                  {reviews.map((review) => (
                    <div key={review.id} className="p-4 bg-[#fdfaf6] rounded-xl border border-[#eddcd2]/50 text-sm">
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-semibold text-[#2d2219] max-w-[150px] truncate" title={review.author}>
                      ...
                      {review.favorite_drinks && (
                        <p className="text-xs font-semibold text-[#8c6239] mb-1 flex items-center gap-1">
                          <Heart className="w-3 h-3 fill-[#8c6239]" /> Recommended: {review.favorite_drinks}
                        </p>
                      )}
            ...
            <div className="bg-[#f5ebe0]/30 rounded-2xl border border-[#eddcd2]/60 p-8 text-center flex flex-col items-center justify-center h-72 border-dashed">
              ...
              <h4 className="font-bold text-[#2d2219]">Select a Bubble Tea Shop</h4>
              <p className="text-xs text-[#7f6a5b] mt-1 max-w-[200px]">
                Click on any shop card to view detailed reviews, and add your own feedback.
              </p>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl border border-[#eddcd2] w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-4">
              {authMode !== 'confirm' ? (
                <>
                  <div>
                    <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Email Address</label>
                    <input
                      type="email"
                      required
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Password</label>
                    <input
                      type="password"
                      required
                      placeholder="••••••••"
                ...
                <div>
                  <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Verification Code</label>
                  <p className="text-xs text-[#7f6a5b] mb-2">We sent a verification code to {authEmail}. Please enter it below.</p>
                  <input
                    type="text"
                    required
                    placeholder="123456"
              ...
              >
                {authLoading ? (
                  ...
                  <>
                    {authMode === 'login' && <><LogIn className="w-4 h-4" /> Sign In</>}
                    {authMode === 'register' && <><UserPlus className="w-4 h-4" /> Register</>}
                    {authMode === 'confirm' && <><Mail className="w-4 h-4" /> Verify Code</>}
                  </>
                )}
              </button>
            </form>
      ...
      {isAddShopOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl border border-[#eddcd2] w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-[#2d2219]">Add a New Bubble Tea Shop</h3>
              <button
            ...
            <form onSubmit={handleAddShop} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Shop Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Origin Country</label>
                <input
                  type="text"
                  required
      ...
      {isAddReviewOpen && selectedShop && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl border border-[#eddcd2] w-full max-w-md p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="text-lg font-bold text-[#2d2219]">Review {selectedShop.name}</h3>
                <p className="text-xs text-[#7f6a5b]">Share your boba experience</p>
              </div>
              <button
            ...
            <form onSubmit={handleAddReview} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Rating</label>
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        className={`w-8 h-8 ${
                          star <= reviewRating ? "text-amber-500 fill-amber-500" : "text-[#eddcd2]"
                        }`}
                      />
                    </button>
                  ))}
              ...
              <div>
                <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Favorite Drink Recommend</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-[#7f6a5b] mb-1">Review Details</label>
                <textarea
                  rows={3}
      ...
      <footer className="w-full border-t border-[#eddcd2]/80 py-6 bg-[#fdfaf6]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-xs text-[#7f6a5b]">
          <p>© 2026 BobaRanker. Crafted with bubble tea passion.</p>
        </div>
      </footer>
    </div>
  );
